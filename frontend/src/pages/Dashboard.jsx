...
const Dashboard = () => {
  ...
  const stats = [
    { title: t('dashboard.totalComplaints'), value: total, icon: <FileText className="text-blue-500" />, bg: 'bg-blue-100' },
    { title: t('dashboard.resolved'), value: resolved, icon: <CheckCircle className="text-emerald-500" />, bg: 'bg-green-100' },
    { title: t('dashboard.pending'), value: pending, icon: <Clock className="text-yellow-500" />, bg: 'bg-yellow-100' },
    { title: t('dashboard.rejected'), value: rejected, icon: <XCircle className="text-red-500" />, bg: 'bg-red-100' },
  ];
  ...
  return (
    <div className="min-h-screen pt-20 bg-background px-4 sm:px-6 lg:px-8 pb-12">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            <h1 className="text-2xl font-bold mb-2">{t('dashboard.welcome')}, {user?.fullName?.split(' ')[0] || t('dashboard.user')}!</h1>
          ...
          <Link to="/submit-complaint" className="mt-4 sm:mt-0 bg-white text-primary px-6 py-2 rounded-lg font-medium shadow-md hover:bg-gray-50 flex items-center transition-colors">
            <Plus className="mr-2 h-5 w-5" /> {t('dashboard.newComplaint')}
          </Link>
        </motion.div>
            ...
            <div className="glass-card p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold flex items-center text-gray-800">
                  <Activity className="mr-2 h-5 w-5 text-primary" /> {t('dashboard.recentComplaints')}
                </h2>
                <Link to="/complaints" className="text-sm text-primary hover:underline font-medium">{t('dashboard.viewAll')}</Link>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-200 text-gray-500 text-sm">
                      <th className="pb-3 font-medium">{t('dashboard.id')}</th>
                      <th className="pb-3 font-medium">{t('dashboard.title')}</th>
                  ...
                  <tbody>
                    {recentComplaints.length === 0 ? (
                      <tr><td colSpan="5" className="py-8 text-center text-gray-500">{t('dashboard.noComplaints')}</td></tr>
                    ) : (
          ...
          <div className="space-y-8">
            ...
            <div className="glass-card p-6">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900">{user?.fullName || t('dashboard.user')}</h3>
              ...
              <div className="border-t border-gray-100 pt-4">
                <Link to="/profile" className="w-full flex items-center justify-center text-sm font-medium text-primary hover:text-secondary py-2 transition-colors">
                  <User className="mr-2 h-4 w-4" /> {t('dashboard.editProfile')}
                </Link>
              </div>
            </div>
            ...
            <div className="glass-card p-6">
              <h2 className="text-xl font-bold flex items-center mb-4 text-gray-800">
                <Bell className="mr-2 h-5 w-5 text-primary" /> {t('dashboard.notifications')}
              </h2>
              <div className="space-y-4">
                <div className="flex items-start space-x-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                  <div className="mt-0.5"><div className="h-2 w-2 bg-primary rounded-full"></div></div>
                  <div>
                    <p className="text-sm font-medium text-gray-800">{t('dashboard.statusUpdated')}</p>
                    <p className="text-xs text-gray-600 mt-1">{t('dashboard.statusUpdatedDesc')}</p>
                    <span className="text-[10px] text-gray-400 mt-1 block">{t('dashboard.twoHoursAgo')}</span>
                  </div>
                </div>
                <div className="flex items-start space-x-3 p-3 hover:bg-gray-50 rounded-lg transition-colors cursor-pointer border border-transparent">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-800">{t('dashboard.assignedToOfficer')}</p>
                    <p className="text-xs text-gray-600 mt-1">{t('dashboard.assignedDesc')}</p>
                    <span className="text-[10px] text-gray-400 mt-1 block">{t('dashboard.oneDayAgo')}</span>
                  </div>
                </div>
              </div>
              <button className="w-full mt-4 text-sm text-center text-gray-500 hover:text-primary transition-colors font-medium">{t('dashboard.viewAll')}</button>
            </div>
          </div>
        </div>
      </div>
